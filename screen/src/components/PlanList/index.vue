...
const props = defineProps(['saleData'])

const config = ref(null)

watch(() => props.saleData, () => {
    const data = []
    const align = []
    const columnIndex = []
    for (let i = 0; i < props.saleData.length; i++) {
        data[i] = []
        align[i] = 'center'
        if (i % 2 === 0) {
        ...
        switch (j) {
            case 0:
            text = props.saleData[i].order
            break
            case 1:
            text = props.saleData[i].shop
            break
            case 2:
            text = props.saleData[i].rider
            break
            case 3:
            text = props.saleData[i].newShop
            break
            case 4:
            text = props.saleData[i].avgOrder
            break
            default:
        }
        if (j === 1 || j === 3) {
